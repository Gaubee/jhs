...
var cache = {
	...
	getFileCache: function(pathname) {
		...
		if (!cache.hasClockCache(file_key)) {
			var pre_state = {};
			cache.defineClockCache(file_key, "time", {
				...
				time_fun: function(_update_cache) {
					...
					if (!equal_state(current_state, pre_state)) {
						if (Object.keys(pre_state).length == 0) {
							console.log("[ file:read ]".colorsHead(), "初始化文件缓存", "=>", pathname, "\n");
						} else {
							console.log("[ file:read ]".colorsHead(), "文件缓存过期", "=>", pathname, "\n");
						}
						pre_state = current_state;
						_update_cache();
					} else {
						console.log("[ file:read ]".colorsHead(), "使用文件缓存", "=>", pathname, "\n");
