	...
	getFileCache: co.wrap(function*(pathname, time_out, jhs_options) {
		...
		if (!cache.hasClockCache(file_key)) {
			...
			cache.defineClockCache(file_key, "time_and_before", {
				get_value_handle: co.wrap(function*(_update_cache, reject) {
					...
					// 读取并执行文件脚本最后得出的内容
					yield fss.readFileMutilAsync(pathname, file_cache, jhs_options);
					// 传入的file_cache
					_update_cache(file_cache);
				}, (err, _update_cache, reject) => reject(err)),
