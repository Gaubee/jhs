...
function _mix_file(file_paths, index, return_er) {
	index = ~~index;
		...
		if (return_er) {
			if (!_file_type_info) {
				...
				if (res === _new_file_placeholder_) {
					res = _mix_file(file_paths, index + 1, return_er);
				} else {
					res = res.toString()
...
fss.readFileSync = function function_name(file_paths, return_er) {
	if (Array.isArray(file_paths)) {
		return _mix_file(file_paths, 0, return_er)
	} else {
		return _readFile(file_paths, return_er)
	}
};

function _readFile(filepath, return_er) {
