	...
	if (!_file_type_info) {
		...
		const _init_sandbox = () => {
			...
			sandbox = {
				...
				__dirname: path.parse(filepath).dir, // TODO:考虑HTTP协议
				get __SUPER() {
					...
					if (isHTTPUrl(import_filepath)) {
						...
						paths = root.map(function(folder_path) {
							if (isHTTPUrl(folder_path)) {
								...
								if (!_f_no_r) {
									// 获取目录
									const _f = filepath.substr(0, filepath.length - sandbox.__filename);
									// 获取跟
									const _r = path.normalize(root[index]);
									// 获取相对目录
									_f_no_r = _f.substr(_r.length);
								}
								return path.normalize(folder_path + path.sep + _f_no_r + path.sep + import_filepath);
							}
						});
					}
					//@6
