...
fss.readFileMutilAsync = function(file_paths, return_er, jhs_options) {
	...
	// try {
	// 	var res = _mix_file(file_paths, 0, return_er, jhs_options).catch(console.error.bind(console))
...
const _mix_file = co.wrap(function*(file_paths, index, return_er, jhs_options) {
	...
	const _file_type_info = filetype(type_buff);
	// 文本类型的file_type为null
	if (!_file_type_info) {
		//文本类型的文件直接读出Buffer
		...
		const _init_sandbox = () => {
			...
			sandbox = {
				...
				console: console,
				__filename: path.basename(filepath), // TODO:考虑HTTP协议
				__dirname: path.normalize(filepath), // TODO:考虑HTTP协议
				get __SUPER() {
				...
				IMPORT: function _IMPORT(import_filepath, _sep) {
					 ...
					 */
					if (import_filepath.indexOf("http://") === 0 || import_filepath.indexOf("https://") === 0) {
						var paths = [import_filepath]
					} else {
						paths = root.map(function(folder_path) {
							if (folder_path.indexOf("http://") === 0 || folder_path.indexOf("https://") === 0) {
								...
								return path.normalize(sandbox.__dirname + path.sep + import_filepath);
							}
						});
					}
					//@6
					...
					return _mix_file(paths, 0, {}, jhs_options).then(import_file => Promise.readStream(import_file.getReadAbleStream()));
				},
		...
		return_er.getReadAbleStream = () => fss.getReadAbleStream(return_er.filepath);
	}
	return return_er;
});
