...
fss.lstatMutilAsync = co.wrap(function*(file_paths) {
	if (Array.isArray(file_paths)) {
		...
		return res_arr.reduce(function(res, lstat_res) {
			Object.keys(lstat_res).forEach(key => {
				res[key] = (res[key] || "") + (lstat_res[key]||"")
			});
			return res;
		}, {});
	} else {
		...
		if (_depends && _depends.length) {
			const res = {
				// 字符串化，可迭代拼接
				current: JSON.stringify(yield _lstat(file_path)),
				DEPENDS: JSON.stringify(yield fss.lstatMutilAsync(_depends))
			};
			return res;
		} else {
...
const file_type_file_str = fs.readFileSync(__dirname + "/filetype.js").toString();
//获取一个文件类型所需要的最大文件头
const file_type_need_size = (() => {
	return file_type_file_str.match(/buf\[(\d+?)\]/g).reduce((res, str) => {
		return Math.max(res, parseInt(str.substring(4, str.length - 1)))
	}, 0);
}());
//获取一个特殊文件的文件后缀
const file_type_map = (() => {
	const res = new Set();
	file_type_file_str.match(/ext: '(\w+?)',/g).forEach(str => {
		res.add("." + str.substring(6, str.length - 2))
	});
	return res;
}());
...
const _mix_file = co.wrap(function*(file_paths, index, return_er, jhs_options) {
	...
	if (return_er.is_url = isHTTPUrl(filepath)) {
		// 如果后缀是特殊文件，那就以特殊文件处理，有的文件服务器给了头部信息是错的
		if (file_type_map.has(path.extname(url.parse(filepath).pathname))) {
			var _file_type_info = {
				mime: "stream/un-save"
			}
		} else {
		...
		Object.defineProperty(return_er, "_folderpath_without_root", {
			...
			get: () => {
				// 获取目录
				const _f = filepath.substr(0, filepath.length - path.basename(filepath).length);
				// 获取跟
				const _r = path.normalize(jhs_options.root[index]);
				// 获取相对目录
				const _f_no_r = _f.substr(_r.length);
				console.log(_f, _f.length, _r, _r.length, _f_no_r);
		...
		const _init_sandbox = () => {
			...
			sandbox = {
				...
				get __filename() {
					...
					Object.defineProperty(this, "__filename", {
						value: res
					});
					return res;
				},
				...
				IMPORT: function _IMPORT(import_filepath, _sep) {
					...
					return _mix_file(paths, 0, {}, jhs_options).then(import_file => import_file && import_file.getReadAbleStream().then(stream => Promise.readStream(stream)));
				},
